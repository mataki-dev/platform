...
func Validate(req SearchRequest, schema ResourceSchema) (ValidatedSearch, []ValidationError) {
	...
	if len(vs.Sort) == 0 && len(req.Sort) == 0 {
		if req.Query != "" && schema.FullText != nil {
			...
			// Resolve Column from the schema's field def so the builder
			// emits a valid SQL identifier. Schemas may declare DefaultSort
			// with only Field set.
			for _, sd := range schema.DefaultSort {
				col := sd.Column
				if col == "" {
					if fd, ok := schema.Fields[sd.Field]; ok {
						col = resolveColumn(sd.Field, fd)
					} else {
						col = sd.Field
					}
				}
